    ...
    return h('div', null, 
        ...
        loans.length === 0
            ...
            : h('ul', null, ...loans.map(l =>
                h('li', { class: 'list-item-spaced' },
                    'Předmět ID: ' + l.itemId + ' (Stav: ' + l.status + ') | Vrátit do: ' + (l.dueDate || 'neurčeno') + ' ',
...
function renderAdmin() {
    return h('div', null, 
        ...
        h('section', { class: 'card' }, 
            ...
            h('table', { role: 'grid' }, 
                h('thead', null, h('tr', null, h('th', null, 'Předmět'), h('th', null, 'Zákazník'), h('th', null, 'Vrátit do'), h('th', null, 'Stav'), h('th', null, 'Akce'))),
        ...
        h('section', { class: 'card' }, 
            ...
            h('ul', null, ...appState.data.items.filter(i => i.status === 'IN_REPAIR').map(i => 
                h('li', null, 'Poškozeno: ' + i.name + ' ', 
                    h('button', { class: 'btn-action', onClick: () => hnd.onFix(i.id) }, 'Opraveno (Vrátit do nabídky)')
                )
            ))
        ),
